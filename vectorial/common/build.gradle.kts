...
val generateFields by tasks.registering {
    val fieldsFile = file("src/generated/java/com/github/uright008/vec/core/GeneratedFields.java")
    val accessorsFile = file("src/generated/java/com/github/uright008/vec/core/GeneratedAccessors.java")
    val syncFile = file("src/generated/java/com/github/uright008/vec/core/GeneratedSync.java")
    ...
    doLast {
        ...
        // Special-case overrides for non-conventional getter/setter names
        ...
        val specialSetters = mapOf(
            ...
            "position" to null, // Vec3.position is a field, set via setPos
        ...
        fun guessSetter(fieldName: String): String? {
            specialSetters[fieldName]?.let { return it } // null means "known, but no setter"
        ...
        // ── Helper: camelCase → UPPER_SNAKE_CASE ──
        fun String.toSnake() = replace(Regex("([a-z])([A-Z])"), "$1_$2").uppercase()

        // ── Compute ordinals ──
        var ordIdx = 0
        val ordinals = mutableMapOf<String, Int>() // fieldName → base ordinal
        for (f in fields) {
            ordinals[f.name] = ordIdx
            ordIdx += when (f.type) {
                "Vec3" -> 3; "AABB" -> 6; else -> 1
            }
        }

        // ── Generate GeneratedFields.java ──
        ...
        for (f in fields) {
            ...
            when (f.type) {
                "double", "float", "int", "boolean" -> {
                    fieldsSb.append("    public static final int $n = ${ordinals[f.name]};\n")
                }
                "Vec3" -> {
        ...
        // ── Generate GeneratedAccessors.java ──
        ...
        accSb.append("        public int ordCount() {\n")
        accSb.append("            return switch (type) {\n")
        accSb.append("                case \"Vec3\" -> 3;\n")
        accSb.append("                case \"AABB\" -> 6;\n")
        accSb.append("                default -> 1;\n")
        accSb.append("            };\n")
        accSb.append("        }\n")
        accSb.append("    }\n")
        ...
        // ── Generate GeneratedSync.java ──
        // Only syncs fields accessible from external code: public fields, or fields with getters
        ...
        syncSb.append("    /** Sync all SoA-captured fields from the entity into SoAStore. Call once per tick. */\n")
        ...
        for (a in accessors) {
            ...
            when (a.type) {
                "double", "float", "int" -> {
                    val src = if (getter != null) "entity.${getter}()"
                              else "entity.${a.fieldName}"
                ...
                "Vec3" -> {
                    // Use getter if available, else skip (can't access private Vec3 fields)
                    if (getter != null) {
                        syncSb.append("        { Vec3 v = entity.${getter}(); if (v != null) { f[$o][slot]=v.x; f[${o+1}][slot]=v.y; f[${o+2}][slot]=v.z; } }\n")
                    }
                }
                "AABB" -> {
                    // Use getter if available, else skip
                    if (getter != null) {
        ...
        logger.lifecycle("GeneratedFields: ${fields.size} fields, ${accessors.count { it.getterName != null }} getters, ${accessors.count { it.setterName != null }} setters from Entity.class via javap")
    }
}
